        ...
        if(item['name'] != "全国" and item['name'] != "非湖北"):
            for item1 in item['cityList']:
                for key in item1['records']:
                    ...
                    sql = "INSERT INTO city_records(name, lastUpdate, updateTime, zipCode, confirmedCount, suspectedCount, curedCount, deadCount, insickCount, confirmedIncreased, curedIncreased, deadIncreased) VALUES (\'" + name + "\',\'" + lastUpdate + "\',\'" + updateTime + "\',\'" + zipCode + "\',\'" + confirmedCount + "\',\'" + suspectedCount + "\',\'" + curedCount + "\',\'" + deadCount + "\',\'" + insickCount + "\',\'" + confirmedIncreased + "\',\'" + curedIncreased + "\',\'" + deadIncreased + "\')"
                    print(sql)
                    cursor.execute(sql)
                    db.commit()
